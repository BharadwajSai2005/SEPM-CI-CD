...
class PatientInput(BaseModel):
    ...
    trestbps: int   = Field(..., ge=80,  le=200, description="Resting BP (mmHg)",      example=125)
    ...
    restecg:  int   = Field(..., ge=0,   le=2,   description="Resting ECG result 0-2", example=1)
    thalach:  int   = Field(..., ge=60,  le=220, description="Max heart rate achieved",example=168)
    exang:    int   = Field(..., ge=0,   le=1,   description="Exercise-induced angina",example=0)
    oldpeak:  float = Field(..., ge=0.0, le=7.0, description="ST depression",          example=1.0)
    ...
    thal:     int   = Field(..., ge=0,   le=3,   description="Thal: 0=norm,1=fixed,2=rev", example=3)
